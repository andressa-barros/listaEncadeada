public class ListaEncadeada {
    private Node Lista;
    public ListaEncadeada(){
        this.Lista = null;
    }

    public void inserir (int informacao){
        Node no = new Node();
        no.setInformacao(informacao);
        if(Lista == null){
            Lista=no;
        }
        else{
            Node atual =Lista;
            while(atual.getProximo()!=null){
                atual = atual.getProximo();
            }
            atual.setProximo(no);
            System.out.println("Nó atual no endereço" + atual);
        }
    }

   public void inserePrimeiro(int informacao){
        Node novoPrimeiro = new Node();
        novoPrimeiro.setInformacao(informacao);

        if (Lista == null){
            Lista=novoPrimeiro;
        }
        else{
            Node primeiro = Lista;
            novoPrimeiro.setProximo(primeiro);
            Lista = novoPrimeiro;
        }
    }

    public void insereDepois(int posicao, int informacao) {
        Node no = new Node();
        int contador = 0;
        Node atual = Lista;
        while (contador < posicao && atual != null) {
            atual = atual.getProximo();
            ++contador;
        }

        if (atual == null) {
            throw new IllegalArgumentException("Posição " + posicao + " é inválida ou não existe na lista.");
        }

        no.setProximo(atual.getProximo());
        no.setInformacao(informacao);
        atual.setProximo(no);
    }


    public void insereOrdenado(int info) {
        Node no = new Node();
        no.setInformacao(info);
        if (Lista == null || Lista.getInformacao() >= info) {
            no.setProximo(Lista);
            Lista = no;
            return;
        }
        Node atual = Lista;
        while (atual.getProximo() != null && atual.getProximo().getInformacao() < info) {
            atual = atual.getProximo();
        }
        no.setProximo(atual.getProximo());
        atual.setProximo(no);
    }

    public void removePrimeiro(){
        if (Lista == null){
            System.out.println("A lista está vazia");
        }
        else{
            Node proximo = Lista.getProximo();
            System.out.println("Valor removido: " + Lista.getInformacao());
            Lista = proximo;
        }
    }

    public void removeUltimo(){
        if (Lista == null){
            System.out.println("A lista está vazia");
        }
        else{
            Node atual = Lista;
            while (atual != null){
                if (atual.getProximo() == null){
                    System.out.println("Valor removido: " + Lista.getInformacao());
                    Lista = null;
                    break;
                }
                if (atual.getProximo() != null && atual.getProximo().getProximo() == null){
                    System.out.println("Valor removido: " + atual.getProximo().getInformacao());
                    atual.setProximo(null);
                    break;
                }
                atual = atual.getProximo();
            }
        }
    }

    public void remove(int indice){
        if (Lista == null){
            System.out.println("Lista vazia");
        }
        else{
            Node atual = Lista;
            Node anterior = null;
            boolean removido = false;
            int aux = 0;
            while (atual != null){
                if (indice == 0 && atual.getProximo() == null){
                    System.out.println("Nó removido no indíce: 0");
                    System.out.println("Nó removido: " + atual.getInformacao());
                    Lista = null;
                    removido = true;
                    break;
                }
                if (indice == 0 && atual.getProximo() != null){
                    System.out.println("Nó removido no indíce: 0");
                    System.out.println("Nó removido: " + atual.getInformacao());
                    Node proximo = atual.getProximo();
                    Lista = proximo;
                    removido = true;
                    break;

                }
                if (aux == indice){
                    System.out.println("Nó removido no indíce: " + indice);
                    System.out.println("Nó removido: " + atual.getInformacao());
                    Node proximo = atual.getProximo();
                    anterior.setProximo(proximo);
                    removido = true;
                    break;
                }
                anterior = atual;
                atual = atual.getProximo();
                aux++;
            }
            if (!removido){
                System.out.println("Índice inválido");
            }
        }
    }

    public void imprime(){
        Node atual = Lista;
        while (atual != null) {
            System.out.print(atual.getInformacao()+ " -> ");
            atual= atual.getProximo();
        }
        System.out.println("Null");
    }

}